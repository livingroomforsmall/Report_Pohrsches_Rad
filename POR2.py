...
x1 = np.array(df.iloc[:, 6] - 2.28, dtype=float)  # Explizit float
y1 = np.array(df.iloc[:, 7] / 98.15 * np.pi, dtype=float)  # Explizit float
valid_data_mask1 = np.isfinite(x1) & np.isfinite(y1)
x1 = x1[valid_data_mask1]
y1 = y1[valid_data_mask1]

# Second dataset (for 0.3A)
x2 = np.array(df.iloc[:, 9] - 2.28, dtype=float)  # Explizit float
y2 = np.array(df.iloc[:, 10] / 98.15 * np.pi, dtype=float)  # Explizit float
...
axes[0].set_title("Auftragung für 0.4A", fontsize=14)
axes[0].grid(True)
...
axes[1].set_title("Auftragung für 0.5A", fontsize=14)
axes[1].grid(True)
