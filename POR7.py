...
# Daten ab Zeile 42 (Index 48)
df = df.iloc[41:].copy()  # Wichtig: .copy(), um eine Kopie zu erstellen

# First dataset (for 0.2A)
x1 = np.array(df.iloc[:, 36] - 1.93, dtype=float)  # Explizit float
y1 = np.array(df.iloc[:, 37] / 98.15 * np.pi, dtype=float)  # Explizit float
valid_data_mask1 = np.isfinite(x1) & np.isfinite(y1)
x1 = x1[valid_data_mask1]
y1 = y1[valid_data_mask1]

# Second dataset (for 0.3A)
x2 = np.array(df.iloc[:, 39] - 1.93, dtype=float)  # Explizit float
y2 = np.array(df.iloc[:, 40] / 98.15 * np.pi, dtype=float)  # Explizit float
...
axes[0].set_title("Auftragung für 1.4A", fontsize=14)
axes[0].grid(True)
...
axes[1].set_title("Auftragung für 1.5A", fontsize=14)
axes[1].grid(True)
