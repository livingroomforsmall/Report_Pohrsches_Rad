...
x1 = np.array(df.iloc[:, 0] - 2.28, dtype=float)  # Explizit float
y1 = np.array(df.iloc[:, 1] / 98.15 * np.pi, dtype=float)  # Explizit float
valid_data_mask1 = np.isfinite(x1) & np.isfinite(y1)
x1 = x1[valid_data_mask1]
y1 = y1[valid_data_mask1]

# Second dataset (for 0.3A)
x2 = np.array(df.iloc[:, 3] - 2.28, dtype=float)  # Explizit float
y2 = np.array(df.iloc[:, 4] / 98.15 * np.pi, dtype=float)  # Explizit float
...
axes[0].set_title("Auftragung für 0.2A", fontsize=14)
axes[0].grid(True)
...
axes[1].set_title("Auftragung für 0.3A", fontsize=14)
axes[1].grid(True)
